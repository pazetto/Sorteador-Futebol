...
export interface JogadorGol {
  ...
  posicao: Posicao;
  gols: number;
  minutosGols: number[]; // minuto de cada gol (ex: [3, 17, 42])
}
...
// ─── Rodada (confrontos pendentes após sorteio) ────────────────────────────

export interface ConfrontoJogado {
  timeId1: string;
  timeId2: string;
  partidaId?: string;
}
...
export interface Rodada {
  ...
  criadaEm: string;
  times: Time[];
  confrontosJogados: ConfrontoJogado[];
}
