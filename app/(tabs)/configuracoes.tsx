  ...
  async function exportarBackup() {
    try {
      ...
      const backup = {
        ...
        app: 'Fut Sorteio',
        bundleId: 'com.pazetto.futsorteio',
        dados: {
        ...
        totais: {
          jogadores: estado.jogadores.length,
          partidas: estado.partidas.length,
          gols: estado.partidas.reduce((acc, p) =>
            acc + p.times.reduce((a, t) => a + t.totalGols, 0), 0),
        },
      };

      const json = JSON.stringify(backup, null, 2);
      const nomeArquivo = `futsorteio-backup-${timestamp}.json`;
      ...
      if (podeCompartilhar) {
        await Sharing.shareAsync(caminho, {
          ...
          dialogTitle: 'Salvar backup do Fut Sorteio',
          UTI: 'public.json',
        });
      } else {
        Alert.alert('Backup gerado', `Arquivo salvo em:\n${caminho}`);
      }
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 16 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{
          ...
          <Text style={{ fontSize: 12, color: colors.muted, marginBottom: 12 }}>
            Exporta jogadores, partidas, artilharia e rodadas em JSON. Fácil de importar no Firebase ou outro banco de dados.
          </Text>
          <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
            <View style={{ flex: 1, backgroundColor: colors.background, borderRadius: 8, padding: 10, alignItems: 'center' }}>
              <Text style={{ fontSize: 20, fontWeight: '700', color: colors.primary }}>{estado.jogadores.length}</Text>
              <Text style={{ fontSize: 11, color: colors.muted }}>Jogadores</Text>
            </View>
            <View style={{ flex: 1, backgroundColor: colors.background, borderRadius: 8, padding: 10, alignItems: 'center' }}>
              <Text style={{ fontSize: 20, fontWeight: '700', color: colors.primary }}>{estado.partidas.length}</Text>
              <Text style={{ fontSize: 11, color: colors.muted }}>Partidas</Text>
            </View>
            <View style={{ flex: 1, backgroundColor: colors.background, borderRadius: 8, padding: 10, alignItems: 'center' }}>
              <Text style={{ fontSize: 20, fontWeight: '700', color: colors.primary }}>
                {estado.partidas.reduce((acc, p) => acc + p.times.reduce((a, t) => a + t.totalGols, 0), 0)}
              </Text>
          ...
          <Pressable
            ...
            style={({ pressed }) => ({
              ...
              opacity: pressed || exportando ? 0.7 : 1,
            })}
          >
